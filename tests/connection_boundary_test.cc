...
std::string MakeLoginFrame(std::uint64_t request_id, std::string_view user,
  ...
  auto frame = hxrpc::RpcCodec::EncodeRequest(
      hxrpc::RpcRequest{request_id, "UserServiceRpc", "Login", {}, payload.value()});
...
int main() {
  ...
  const auto partial_response = hxrpc::RpcCodec::DecodeResponse(ReceiveFrame(fd));
  ...
  const auto second_frame = MakeLoginFrame(9, "alice", "bad-password", serializer);
  ...
  const auto second_response = hxrpc::RpcCodec::DecodeResponse(ReceiveFrame(fd));
