...
int main() {
  // 测试目的: 集中覆盖三类失败路径——服务发现失败, 未知方法, 非法大包
  ...
  // 场景一: 清空静态服务列表, 验证客户端返回 discovery error
  ...
  assert(missing_instance.error().code ==
         hxrpc::RpcStatusCode::kDiscoveryError);

  // 场景二: 直接发送不存在的方法名, 验证服务端返回 kUnknownMethod
  ...
  const auto method_response =
      hxrpc::RpcCodec::DecodeResponse(ReceiveFrame(unknown_method_fd));
