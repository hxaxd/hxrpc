...
void hxrpcProvider::NotifyService(google::protobuf::Service *service) {
...
void hxrpcProvider::Run() {
  // 读取配置文件中的RPC服务器IP和端口
  std::string ip =
      hxrpcApplication::GetInstance().GetConfig().Load("rpcserverip");
  int port = atoi(hxrpcApplication::GetInstance()
                      .GetConfig()
                      .Load("rpcserverport")
                      .c_str());
  ...
  server->setConnectionCallback(
      std::bind(&hxrpcProvider::OnConnection, this, std::placeholders::_1));
  server->setMessageCallback(
      std::bind(&hxrpcProvider::OnMessage, this, std::placeholders::_1,
...
// 连接回调函数，处理客户端连接事件
void hxrpcProvider::OnConnection(const muduo::net::TcpConnectionPtr &conn) {
  if (!conn->connected()) {
    // 如果连接关闭，则断开连接
    conn->shutdown();
  }
...
void hxrpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn,
  ...
  while (buffer->readableBytes() >= 4) {
    ...
    // 4. 业务逻辑处理
    if (!hxrpcHeader.ParseFromString(rpc_header_str)) {
      std::cout << "header parse error" << std::endl;
      return;
    }

    std::string service_name = hxrpcHeader.service_name();
    std::string method_name = hxrpcHeader.method_name();
    ...
    google::protobuf::Closure *done =
        google::protobuf::NewCallback<hxrpcProvider,
                                      const muduo::net::TcpConnectionPtr &,
                                      google::protobuf::Message *>(
            this, &hxrpcProvider::SendRpcResponse, conn, response);
    service->CallMethod(method, nullptr, request, response, done);
  }
}

// 发送RPC响应给客户端
void hxrpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr &conn,
                                    google::protobuf::Message *response) {
...
// 析构函数，退出事件循环
hxrpcProvider::~hxrpcProvider() {
  std::cout << "~hxrpcProvider()" << std::endl;
