...
ssize_t hxrpcChannel::recv_exact(int fd, char *buf, size_t size) {
  size_t total_read = 0;
...
void hxrpcChannel::CallMethod(
  ...
  if (-1 == m_clientfd) { // 如果客户端socket未初始化
    ...
    std::string host_data = QueryServiceHost(&zkCli, service_name, method_name,
                                             m_idx); // 查询服务地址
  ...
  hxrpcheader.set_service_name(service_name);
  hxrpcheader.set_method_name(method_name);
  hxrpcheader.set_args_size(args_str.size());

  std::string rpc_header_str;
  if (!hxrpcheader.SerializeToString(&rpc_header_str)) {
  ...
  uint32_t total_len =
      4 + header_size +
      args_str.size(); // Total Len 包含 HeaderLen(4) + Header + Body
...
// 创建新的socket连接
bool hxrpcChannel::newConnect(const char *ip, uint16_t port) {
  // 创建socket
...
// 从ZooKeeper查询服务地址
std::string hxrpcChannel::QueryServiceHost(ZkClient *zkclient,
...
// 构造函数，支持延迟连接
hxrpcChannel::hxrpcChannel(bool connectNow) : m_clientfd(-1), m_idx(0) {
