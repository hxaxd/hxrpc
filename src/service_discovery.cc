...
std::expected<ServiceInstance, RpcError>
StaticResolver::Resolve(std::string_view service_name, std::string_view method_name) {
  ...
  if (it == config_.static_services.end()) {
    return std::unexpected(
        RpcError{RpcStatusCode::kDiscoveryError, "missing static service: " + key});
  }
...
std::expected<ServiceInstance, RpcError>
StaticResolver::ChooseInstance(const std::vector<Endpoint> &endpoints) {
  if (endpoints.empty()) {
    return std::unexpected(
        RpcError{RpcStatusCode::kDiscoveryError, "no static instance available"});
  }
  const auto &endpoint =
...
NoopRegistrar::Register(std::string_view service_name, std::string_view method_name,
...
ZkResolver::ZkResolver(std::unique_ptr<ZkClient> client)
    : client_(std::move(client)), cache_state_(std::make_shared<CacheState>()),
      random_engine_(std::random_device{}()) {}

std::expected<ServiceInstance, RpcError>
ZkResolver::Resolve(std::string_view service_name, std::string_view method_name) {
...
std::string ZkResolver::BuildInstancePath(std::string_view service_name,
  ...
  return BuildMethodPath(service_name, method_name) + "/" + std::string(instance_id);
}

std::expected<Endpoint, RpcError> ZkResolver::ParseEndpoint(std::string_view raw) {
  ...
  try {
    endpoint.port =
        static_cast<std::uint16_t>(std::stoul(std::string(raw.substr(separator + 1))));
  } catch (...) {
    return std::unexpected(
...
std::expected<ServiceInstance, RpcError>
ZkResolver::ChooseInstance(const std::vector<ServiceInstance> &instances) {
  if (instances.empty()) {
    return std::unexpected(
        RpcError{RpcStatusCode::kDiscoveryError, "no zookeeper instance available"});
  }
  return ChooseItem(instances, ResolverSelectionPolicy::kRandom, random_engine_);
}

std::expected<std::vector<ServiceInstance>, RpcError>
ZkResolver::RefreshMethod(std::string_view service_name, std::string_view method_name) {
  const std::string method_path = BuildMethodPath(service_name, method_name);
...
std::expected<void, RpcError>
ZkRegistrar::Register(std::string_view service_name, std::string_view method_name,
                      const Endpoint &endpoint) {
  const std::string service_path = ZkResolver::BuildServicePath(service_name);
  const std::string method_path = ZkResolver::BuildMethodPath(service_name, method_name);
  ...
  return client_->CreateNode(instance_path.c_str(), instance_id.c_str(),
                             static_cast<int>(instance_id.size()), ZOO_EPHEMERAL);
}

std::unique_ptr<ServiceResolver>
DiscoveryFactory::CreateResolver(const DiscoveryConfig &config) {
...
std::unique_ptr<ServiceRegistrar>
DiscoveryFactory::CreateRegistrar(const DiscoveryConfig &config) {
  if (config.backend == DiscoveryBackend::kZookeeper) {
    auto client = std::make_unique<ZkClient>();
    auto start_result = client->Start(config.zookeeper_endpoint);
    if (!start_result) {
